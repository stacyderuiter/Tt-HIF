---
title: "Supplemental Materials for: Heat increment of feeding in the common bottlenose dolphin (*Tursiops truncatus*) contributes moderately to field metabolic rate estimates"
author:
  - name: I. Koliopoulou
    affiliations:
      - ref: LU
...
affiliations:
  ...
  - id: FO
    ...
    state: Spain
  - id: UBC
    name: Department of Zoology, Univeristy of British Columbia
    city: Vancouver
...
HIF_data <- read_xlsx('data/updated_data.xlsx',
  ...
  mutate(animal = factor(animal),
         animal2 = fct_reorder(animal, kcal, min),
...
```{r}
HIF_ave <- HIF_data |> 
  group_by(animal2, time, kcal) |>
  summarise(oxygen_cons = mean(oxygen_cons, na.rm = TRUE),
            exact = mean(exact, na.rm = TRUE),
            .groups = "drop")

oxygen_time_ind_plot <- 
  ggplot(HIF_data, 
  ...
  facet_wrap("animal2", ncol = 2) +
  geom_point(aes(x = exact, y = oxygen_cons, color = factor(kcal)),
             data = HIF_ave) +
  geom_line(aes(x = exact, y = oxygen_cons, color = factor(kcal)),
            data = HIF_ave) +
  labs(x = "Time (min)", 
...
```{r}
#| label: data-by-individual
#| fig-width: 7.5
#| fig-height: 9.5

print(oxygen_time_ind_plot)
```

Oxygen consumption as a function of time, by individual dolphin. Colors indicate energy intake in kcal. At each timepoint, varying symbols for each dolphin plot individual replicate measurements, while dots plot means at each timepoint. Nominal timepoints were 0, 30, 60, and 120 minutes, but observations are plotted at the exact time they were actually taken (or, for means, the average of those exact times). 
:::
...
Below is the model summary table:
...
```{r}
#| label: suppfig-resid-hist

gf_dhistogram(~resid(HIF_model), bins = 15) |> 
  gf_fitdistr(dist = "dnorm") |>
  gf_labs(x = "GAMM Residuals",
          y = "Density")
```
...
::: {#suppfig-scaled-resids}

```{r}
#| label: suppfig-scaled-resids
#| message: false

plotResiduals(simulateResiduals(HIF_model), quantreg = FALSE)
```

GAMM scaled residual plot
:::
...
acf(resid(HIF_model), main = "")
```

GAMM Residual ACF plot.
:::
...
```{r}
my_atab <- anova(HIF_model)$s.table
terms <- rownames(my_atab)
tibble(Term = terms,
       EDF = my_atab[,"edf"],
       `Reference DF` = my_atab[,"Ref.df"],
       F = my_atab[,"F"],
       `p-value` = my_atab[,"p-value"]
       ) |>
  gt()
```

ANOVA table for GAMM.
:::
...
Variance components associated with each term in the GAMM.
:::
...
Marginal means of data variables.
:::
...
::: {#suppfig-sim-total-vo2}

```{r}
#| label: suppfig-sim-total-vo2
...
Expected liters of oxygen consumed according to simulations.
:::

::: {#suppfig-vo2-total}

```{r}
#| label: suppfig-vo2-total
...
Expected HIF as a percent of daily BMR, according to simulations.
:::
...
This repeats the data-and-model figure from the main manuscript (to show code).
:::

```{r}
ragg::agg_tiff("figures/figure-1.tiff", 
               width = 6, 
               height = 4.5, 
               units = "in", 
               res = 300)
